...
| **P3+c — Push fichiers** | `pousser_fichiers_item` : upload des nouveaux/modifiés via `uploader_fichier`, `PUT /datas/{id}` avec le jeu cible `files[]`, mise à jour `sha1_nakala`. **Garde-fous obligatoires** : refus par défaut si `orphelins_distants` ou `nakala_only_sans_local` (`PUT` retire automatiquement les fichiers absents de la liste → catastrophique pour items publiés). Flag `--retirer-orphelins` pour autoriser explicitement. Smoke live sur apitest pour confirmer le comportement publié vs pending. | futur | élevé |
