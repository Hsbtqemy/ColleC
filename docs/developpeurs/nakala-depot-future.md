...
| **P3+c.2 — CLI + smoke live** | CLI `archives-tool nakala pousser-fichiers <cote> --fonds X [--no-dry-run] [--retirer-orphelins]`. Aperçu text/json du plan (catégorisation, sha1 tronqués, action prévue). Smoke live opt-in apitest : cycle complet dépôt → modif binaire → pousser-fichiers → re-comparer → tout inchangé. | futur | moyen |
